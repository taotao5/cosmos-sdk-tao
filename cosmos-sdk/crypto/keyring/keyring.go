...
type Keyring interface {
	...
	LegacyKey(uid string) (LegacyInfo, error)
...
func (ks keystore) LegacyKeyByAddress(address sdk.Address) (LegacyInfo, error) {
	ik, err := ks.db.Get(addrHexKeyAsString(address))
	if err != nil {
...
func (ks keystore) NewLegacyAccount(name string, mnemonic string, bip39Passphrase string, hdPath string, algo SignatureAlgo) (LegacyInfo, error) {
	...
	// if found
	address := sdk.AccAddress(privKey.PubKey().Address())
	if _, err := ks.LegacyKeyByAddress(address); err == nil {
...
func (ks keystore) LegacyKey(uid string) (LegacyInfo, error) {
...
func (ks keystore) writeLegacyLocalKey(name string, priv types.PrivKey, algo hd.PubKeyType) (LegacyInfo, error) {
...
func (ks keystore) writeInfo(info LegacyInfo) error {
	...
	exists, err := ks.existsInDbInfo(info)
	if err != nil {
		return err
	}
	if exists {
...
func (ks keystore) existsInDbInfo(info LegacyInfo) (bool, error) {
...
func (ks keystore) key(key string) (LegacyInfo, error) {
	if !strings.HasSuffix(key, infoSuffix) {
		key = infoKey(key)
	}
	bs, err := ks.db.Get(key)
	if err != nil {
	...
	return unMarshalLegacyInfo(bs.Data)
}
